defmodule Muzak.Code.Formatter do
  ...
  # This is mostly copied over from Elixir core and just modified a bit to expose some of the
  # internals that I needed.
  ...
  @doc false
  def to_forms_and_state(string, opts \\ []) when is_binary(string) and is_list(opts) do
    ...
    tokenizer =
      if function_exported?(:elixir, :string_to_tokens, 5) do
        &apply(:elixir, :string_to_tokens, [&1, &2, 1, &3, &4])
      else
        &apply(:elixir, :string_to_tokens, [&1, &2, &3, &4])
      end

    with {:ok, tokens} <- tokenizer.(charlist, line, file, tokenizer_options),
  ...
  def to_ast(string, opts \\ []) do
    string |> to_forms_and_state(opts) |> elem(0)
  end

  def to_string(forms, state), do: forms |> to_algebra(state) |> to_string()

  def to_algebra(string) do
    {forms, state} = to_forms_and_state(string)
    to_algebra(forms, state)
  end

  def to_algebra(forms, state) do
    {doc, _} = block_to_algebra(forms, state, @min_line, @max_line)
    doc |> Inspect.Algebra.format(@max_line)
  end
  ...
  defp quoted_to_algebra({:__block__, meta, _} = block, _context, state) do
    {block, state} = block_to_algebra(block, state, line(meta), closing_line(meta))
  ...
  def block_to_algebra(type_fun, state, min_line \\ @min_line, max_line \\ @max_line)

  def block_to_algebra([{:->, _, _} | _] = type_fun, state, min_line, max_line) do
    type_fun_to_algebra(type_fun, min_line, max_line, state)
  end

  def block_to_algebra({:__block__, _, []}, state, min_line, max_line) do
    block_args_to_algebra([], min_line, max_line, state)
  end

  def block_to_algebra({:__block__, _, [_, _ | _] = args}, state, min_line, max_line) do
    block_args_to_algebra(args, min_line, max_line, state)
  end

  def block_to_algebra(block, state, min_line, max_line) do
  ...
  defp module_attribute_to_algebra(meta, {name, call_meta, [_] = args} = expr, context, state)
    ...
    if classify(name) == :callable_local do
  ...
  defp list_interpolation_to_algebra([entry | entries], escape, state, acc, last) do
    ...
    {doc, state} = block_to_algebra(quoted, state, line(meta), closing_line(meta))
    doc = surround("\#{", doc, "}")
  ...
  defp clauses_to_algebra(other, min_line, max_line, state) do
    case block_to_algebra(other, state, min_line, max_line) do
  ...
  defp clause_to_algebra({:->, meta, [[], body]}, _min_line, state) do
    {body_doc, state} = block_to_algebra(body, state, line(meta), closing_line(meta))
  ...
  defp clause_to_algebra({:->, meta, [args, body]}, min_line, state) do
    ...
    {body_doc, state} = block_to_algebra(body, state, min_line, closing_line(meta))
  ...
  defp classify(atom) when is_atom(atom) do
    ...
    cond do
      ...
      Code.Identifier.unary_op(atom) != :error or Code.Identifier.binary_op(atom) != :error ->
        :callable_operator
  ...
  defp inspect_as_function(atom) when is_atom(atom) do
